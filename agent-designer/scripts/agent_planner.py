...
这个文件在整个 skill 里的作用，可以简单理解为：
"把模型整理好的需求，自动变成可执行的架构设计方案和架构图"。

整体流程可以分成 4 步：

1. 模型先参考 `references/` 里的设计说明
   了解常见架构模式、角色分工、通信方式和设计原则。

2. 再按照 `assets/` 里的标准示例结构，生成一份需求 JSON
   这份 JSON 会把目标、任务、限制条件、团队规模等信息整理成统一格式。

3. `agent_planner.py` 读取这份 JSON，并按代码里预设的规则进行判断
   自动选择合适的多智能体架构，拆分 agent 角色、职责、能力、工具和通信关系。

4. 最后输出结构化结果
   包括架构方案、角色定义、实施路线图，以及 Mermaid 格式的架构图文本，
   方便继续展示、评审或落地实现。

所以它之所以能“自动生成架构和架构图”，本质上是因为：
- 前面有参考标准，告诉模型该怎么组织需求
- 中间有标准 JSON，保证输入格式统一
- 后面有明确的规则引擎，负责把输入转换成架构结果
- 最终再把架构结果转成 Mermaid 图
